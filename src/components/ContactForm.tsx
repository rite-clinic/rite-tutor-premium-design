...
const isValidPhone10 = (v: string) => {
  ...
  return normalizeDigits(v).length === 10;
};
...
export function ContactForm({ onSuccess, compact = false }: ContactFormProps) {
  ...
  const handleChange = (
    ...
    if (name === "phone") {
      ...
      if (touched.phone) setErrors((prev) => ({ ...prev, phone: validate({ ...formData, phone: digits }).phone }));
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTouched({
      parent_name: true, email: true, phone: true, child_age: true,
    ...
    try {
      ...
      await axios.post("https://ritetutor.com/backend/api/strategycall/leads/create/", {
        ...
        educational_objectives: payload.educational_objectives.trim(),
        referral_source: payload.referral_source.trim(),
      ...
      <form className="space-y-4" onSubmit={handleSubmit} noValidate>
        ...
        <div className="space-y-1">
          <Input name="phone" value={formData.phone} onChange={handleChange} onBlur={handleBlur}
            placeholder="Phone Number (optional, 10 digits)" className={`h-12 ${fieldError("phone") ? "border-destructive" : ""}`}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-1">
            <Input name="child_age" value={formData.child_age} onChange={handleChange} onBlur={handleBlur}
              placeholder="Child's Age *" className={`h-12 ${fieldError("child_age") ? "border-destructive" : ""}`} required inputMode="numeric" />
