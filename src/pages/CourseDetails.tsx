...
const CourseDetails = () => {
  ...
  const initialCourse = initialCourses?.find(item => String(item.id) === courseId);
  ...
  }, [courseId, reloadKey, initialCourse]);
  ...
  return (
    <>
      ...
      ========================== */}
      <Seo
        title={`${isLoading ? "Online Course" : error?.notFound ? "Course Not Available" : title} | Rite Tutor`}
        description={course ? `${course.title}. ${course.subtitle || course.intro || ""} One-to-one online learning with Rite Tutor. Discuss lesson options and book a free strategy call.` : "Explore one-to-one online courses with Rite Tutor. Discuss your child's subject, grade, and learning goals."}
        path={`/courses/${courseId}`}
        image={course?.image || "/og-image.png"}
        imageAlt={title}
        noindex={!!error || (!course && !isLoading)}
        breadcrumbs={[{ name: "Home", path: "/" }, { name: "Courses", path: "/courses" }, { name: title, path: `/courses/${courseId}` }]}
        schemas={course ? [{ "@type": "Course", "@id": `${absoluteUrl(`/courses/${course.id}`)}#course`, name: title, description: course.subtitle || course.intro || title, provider: { "@id": ORGANIZATION_ID }, url: absoluteUrl(`/courses/${course.id}`), hasCourseInstance: { "@type": "CourseInstance", courseMode: "online" } }] : []}
      />
